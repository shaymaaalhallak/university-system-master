      ...
      Promise.all([api.get<any>("/fees/my"), api.get<any>("/fees/structure")])
        .then(([feesRes, structRes]) => {
          if (feesRes.success) {
            ...
            setTotals({
              paid: feesRes.data.totalPaid || 0,
              pending: feesRes.data.totalPending || 0,
            });
          }
          ...
          <div className="bg-white rounded-xl shadow-sm border p-6">
            ...
            <div className="mb-4">
              <p className="text-gray-700 font-medium">
                Tuition: ${structure.tuition.perSemester} per semester
              </p>
              <p className="text-sm text-gray-500 mb-3">
                {structure.tuition.description}
              </p>
                ...
                {payments.map((p) => (
                  <tr key={p.payment_id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-semibold ${p.status === "completed" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}
                      >
                        {p.status}
                      </span>
                    </td>
