...
export default function StudentGrades() {
  ...
  useEffect(() => {
    if (user?.role === "student") {
      Promise.all([api.get<any>("/grades/my"), api.get<any>("/grades/my-gpa")])
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6 bg-white rounded-xl shadow-sm border p-6">
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                {gpaData && (
                  <p className={`text-sm font-medium ${standingColor()}`}>
                    {gpaData.standing}
                  </p>
                )}
              </div>
        ...
        {gpaData?.semesterGPA &&
          Object.keys(gpaData.semesterGPA).length > 0 && (
              ...
              <div className="flex items-start justify-between gap-4 mb-4">
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    {grade.course_code} • {grade.year} {grade.semester} •{" "}
                ...
                <div className="text-right">
                  ...
                  <span
                    className={`mt-1 inline-flex px-3 py-1 rounded-full text-xs font-semibold ${getGradeColor(grade.letter_grade)}`}
                  >
                    {grade.letter_grade || "-"}
                  </span>
                </div>
        ...
        {gpaData && (
          <div
            className={`rounded-xl p-4 border ${gpaData.canGraduate ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"}`}
          >
            <p
              className={`font-medium ${gpaData.canGraduate ? "text-green-700" : "text-red-700"}`}
            >
              {gpaData.canGraduate
