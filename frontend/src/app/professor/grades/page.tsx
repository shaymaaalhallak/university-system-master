...
export default function ProfessorGradesPage() {
      ...
      current.map((component, currentIndex) =>
        currentIndex === index
          ? {
              ...component,
              [field]:
                field === "weight" ? Math.max(0, Number(value) || 0) : value,
            }
          : component,
      ),
    );
  ...
  const saveStructure = async () => {
    ...
    try {
      ...
      if (!response.success)
        throw new Error(response.message || "Failed to save grade structure");
      ...
      console.error(err);
      setError(
        "Unable to save the grade structure. Make sure names are filled and total weight equals 100.",
      );
    } finally {
  ...
  const saveStudent = async (student: StudentRow) => {
    ...
    try {
      ...
      const componentScores = components.map((component) => ({
        ...
        score: Number(
          student.componentScores[String(component.componentId)] ?? 0,
        ),
      }));
      ...
      setMessage(
        `${student.first_name} ${student.last_name}'s grades were saved.`,
      );
    } catch (err) {
      console.error(err);
      ...
      <div className="space-y-6 text-black bg-white max-h-full">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">
                Create any number of exams, quizzes, projects, or assignments
                for a section. The total weight must equal 100.
              </p>
            </div>
          </div>
        </div>
        ...
        {selectedSectionId && (
          <div className="rounded-xl border bg-white p-6 shadow-sm space-y-4">
            ...
            <div className="space-y-3">
              {components.map((component, index) => (
                <div
                  ...
                  className="grid gap-3 md:grid-cols-[1fr_140px_80px] items-end"
                >
                  <div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateComponent(index, "weight", e.target.value)
                      }
                      className="w-full rounded-lg border px-3 py-2"
                    />
                  </div>
            ...
            <div
              className={`rounded-lg px-3 py-2 text-sm ${totalWeight === 100 ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"}`}
            >
