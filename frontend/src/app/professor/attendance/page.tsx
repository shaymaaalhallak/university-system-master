...
const STATUS_CYCLE: AttendanceRecord["status"][] = ["Present", "Absent", "Late"];
    ...
    const loadStudentsAndAttendance = async () => {
      try {
        ...
        const [studentsResponse, attendanceResponse] = await Promise.all([
          api.get<{ success: boolean; data: Student[] }>(`/attendance/section/${selectedSectionId}/students`),
          api.get<{ success: boolean; data: AttendanceRecord[] }>(
            `/attendance?sectionId=${selectedSectionId}&date=${selectedDate}`
          ),
        ]);

        const enrolledStudents = studentsResponse.success ? studentsResponse.data ?? [] : [];
        const attendanceRows = attendanceResponse.success ? attendanceResponse.data ?? [] : [];
    ...
    <div className="space-y-6 text-black bg-white">
      <div className="rounded-xl border bg-white p-6 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Professor Attendance</h1>
      ...
      <div className="rounded-xl border bg-white p-6 shadow-sm">
        <h2 className="mb-4 text-lg font-semibold text-gray-900">My Sections</h2>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-medium text-gray-900">
                    {section.course_title} <span className="text-sm text-gray-400">({section.course_code})</span>
                  </p>
                  <p className="text-sm text-gray-500">
                    Section {section.section_id} • {section.schedule_time} • {section.room_number} •{" "}
              ...
              <span
                className={`rounded px-2 py-1 text-xs font-medium ${
                  section.grade_entry_enabled ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"
                }`}
              >
                {section.grade_entry_enabled ? "Grades open" : "Grades closed"}
              </span>
            </div>
          ))}
          {!allSections.length && (
            <p className="py-4 text-center text-gray-500">No sections assigned yet.</p>
          )}
        </div>
      </div>
      ...
      <div className="rounded-xl border bg-white p-6 shadow-sm">
        ...
        {!loadingStudents && !selectedSectionId && (
          <p className="text-gray-500">Select a course and section to start attendance.</p>
        )}
