...
const ensureProfessorCanAccessSection = async (userId: number, sectionId: number | string) => {
...
const getSectionComponents = async (sectionId: number | string): Promise<GradeComponent[]> => {
  const rows = await query(
...
const validateComponents = (components: Array<{ name: string; weight: number }>) => {
...
router.get("/my-gpa", verifyToken, requireRole("student"), async (req: Request, res: Response) => {
  try {
    const studentId = await getStudentId(req.user!.id);
    if (!studentId) return res.status(404).json({ success: false, message: "Student profile not found" });
...
router.get("/:studentId/gpa", verifyToken, async (req: Request, res: Response) => {
  try {
...
router.get("/section/:sectionId/setup", verifyToken, requireRole("professor", "admin"), async (req: Request, res: Response) => {
  try {
...
router.put("/section/:sectionId/setup", verifyToken, requireRole("professor", "admin"), async (req: Request, res: Response) => {
  try {
    const { sectionId } = req.params;
    ...
    const components = (req.body.components ?? []).map((component: any, index: number) => ({
      name: String(component.name || "").trim(),
      weight: Number(component.weight),
...
router.post("/", verifyToken, requireRole("professor", "admin"), async (req: Request, res: Response) => {
  try {
    ...
    (Array.isArray(componentScores) ? componentScores : []).forEach((entry: any) => {
    ...
    if (gradeId) {
      await query(
        `UPDATE grades
         ...
         WHERE grade_id = ?`,
        [legacyScores[0], legacyScores[1], legacyScores[2], totalScore, letterGrade, gradeId]
      );
    } else {
      const gradeResult: any = await query(
        `INSERT INTO grades (student_id, section_id, assignment_score, midterm_score, final_score, total_score, letter_grade)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [studentId, sectionId, legacyScores[0], legacyScores[1], legacyScores[2], totalScore, letterGrade]
      );
