    ...
    try {
      ...
      if (req.user!.role === "professor") {
        ...
        const entryControlRows = await query(
          "SELECT is_enabled, close_at, entry_mode FROM grade_entry_control WHERE section_id = ? LIMIT 1",
          [sectionId],
        );
        ...
        if (row.close_at) {
          ...
          if (closeTime < Date.now()) {
            await query(
              "UPDATE grade_entry_control SET is_enabled = 0 WHERE section_id = ?",
              [sectionId],
            );
            return res.status(403).json({
              success: false,
        ...
        if (componentScores && Array.isArray(componentScores) && componentScores.length > 0) {
          const componentIds = componentScores.map((cs: any) => cs.componentId);
          const idPlaceholders = componentIds.map(() => "?").join(",");
          const componentRows = await query(
            `SELECT component_id, component_name FROM grade_components WHERE component_id IN (${idPlaceholders})`,
            componentIds,
          );
          const compNameMap = new Map<number, string>();
          componentRows.forEach((r: any) => compNameMap.set(Number(r.component_id), r.component_name));

          // Get previously saved scores for this student+section
          const enrollment = await query(
            "SELECT enrollment_id FROM enrollments WHERE student_id = ? AND section_id = ? AND status = 'active' LIMIT 1",
            [studentId, sectionId],
          );
          const prevScores = new Map<number, number>();
          if (enrollment.length > 0) {
            const prevRows = await query(
              `SELECT gcs.component_id, gcs.score
               FROM grade_component_scores gcs
               JOIN grades g ON gcs.grade_id = g.grade_id
               WHERE g.student_id = ? AND g.section_id = ?`,
              [studentId, sectionId],
            );
            prevRows.forEach((r: any) => prevScores.set(Number(r.component_id), Number(r.score)));
          }
