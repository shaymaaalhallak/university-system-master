...
router.get("/sections", verifyToken, async (req: Request, res: Response) => {
  try {
    ...
    if (semester) { sql += " AND cs.semester = ?"; params.push(semester); }
    if (year) { sql += " AND cs.year = ?"; params.push(year); }
    if (courseId) { sql += " AND cs.course_id = ?"; params.push(courseId); }
    if (professorId) { sql += " AND cs.professor_id = ?"; params.push(professorId); }
...
router.post("/", verifyToken, requireRole("admin"), async (req: Request, res: Response) => {
  try {
    ...
    const result: any = await query(
      ...
      [courseCode, courseTitle, credits, description, departmentId || null, programId || null]
    );
...
router.put("/:id", verifyToken, requireRole("admin"), async (req: Request, res: Response) => {
  try {
    ...
    await query(
      ...
      [courseCode, courseTitle, credits, description, departmentId, programId, req.params.id]
    );
...
router.post("/sections", verifyToken, requireRole("admin"), async (req: Request, res: Response) => {
  try {
...
router.post("/:id/prerequisites", verifyToken, requireRole("admin"), async (req: Request, res: Response) => {
  try {
...
router.delete("/:id/prerequisites/:reqId", verifyToken, requireRole("admin"), async (req: Request, res: Response) => {
  try {
    await query(
