...
router.get("/student", verifyToken, requireRole("student"), async (req: Request, res: Response) => {
  try {
...
router.get("/professor", verifyToken, requireRole("professor"), async (req: Request, res: Response) => {
  try {
    ...
    const totalStudents = sections.reduce((sum: number, s: any) => sum + Number(s.enrolled), 0);
...
router.get("/admin", verifyToken, requireRole("admin"), async (req: Request, res: Response) => {
  try {
    ...
    try { enrollmentsCount = await query("SELECT COUNT(*) AS count FROM enrollments WHERE status = 'active'"); } catch {}
    try {
    ...
    try { pendingExemptions = await query("SELECT COUNT(*) AS count FROM exam_exemptions WHERE status = 'pending'"); } catch {}
