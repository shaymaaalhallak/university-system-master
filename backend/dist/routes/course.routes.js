...
const isColumnNullable = async (tableName, columnName) => {
    const rows = await query(`SELECT IS_NULLABLE AS is_nullable
     FROM INFORMATION_SCHEMA.COLUMNS
...
router.get("/sections", auth_1.verifyToken, async (req, res) => {
    ...
    `;
        ...
        if (req.user?.role === "student") {
            const studentRows = await query("SELECT student_id, department_id, program_id, semester FROM students WHERE user_id = ? LIMIT 1", [req.user.id]);
...
router.get("/study-plans/meta", auth_1.verifyToken, (0, auth_1.requireRole)("admin"), async (_req, res) => {
    try {
...
router.post("/study-plans", auth_1.verifyToken, (0, auth_1.requireRole)("admin"), async (req, res) => {
    try {
        ...
        let safeDepartmentId = normalizeId(departmentId);
        let safeProgramId = normalizeId(programId);
        if (safeDepartmentId !== null) {
            const departmentRows = await query("SELECT department_id FROM departments WHERE department_id = ? LIMIT 1", [safeDepartmentId]);
            if (!departmentRows.length)
                safeDepartmentId = null;
        }
        if (safeProgramId !== null) {
            const programRows = await query("SELECT program_id FROM programs WHERE program_id = ? LIMIT 1", [safeProgramId]);
        ...
        if (hasDepartmentColumn &&
            ...
            return res.status(400).json({
                success: false,
                message: "A valid department is required for this study-plans schema.",
            });
        }
        ...
        let insert;
        try {
            insert = await query(`INSERT INTO study_plans (${columns.join(", ")}) VALUES (${placeholders.join(", ")})`, values);
        }
        catch (insertError) {
            if (columns.length > 1) {
                try {
                    ...
                    insert = await query(`INSERT INTO study_plans (${fallbackColumns.join(", ")}) VALUES (${fallbackPlaceholders.join(", ")})`, fallbackValues);
                }
                catch (fallbackInsertError) {
                    console.error("Create study plan fallback insert error:", fallbackInsertError);
                    return res.status(400).json({
                        success: false,
                        message: fallbackInsertError?.sqlMessage ||
                ...
                return res.status(400).json({
                    success: false,
                    message: insertError?.sqlMessage ||
                        insertError?.message ||
                        "Unable to create study plan",
                });
            }
...
router.get("/study-plans/:id", auth_1.verifyToken, (0, auth_1.requireRole)("admin"), async (req, res) => {
    try {
        const [hasDepartmentColumn, hasProgramColumn, hasPlanNameColumn] = await Promise.all([
            columnExists("study_plans", "department_id"),
            columnExists("study_plans", "program_id"),
            columnExists("study_plans", "plan_name"),
        ]);
...
router.put("/study-plans/:id", auth_1.verifyToken, (0, auth_1.requireRole)("admin"), async (req, res) => {
    try {
        const { planName, departmentId, programId } = req.body;
        const hasPlanNameColumn = await columnExists("study_plans", "plan_name");
        const planNameColumn = hasPlanNameColumn ? "plan_name" : "name";
        await query(`UPDATE study_plans SET ${planNameColumn} = ?, department_id = ?, program_id = ? WHERE plan_id = ?`, [planName, departmentId || null, programId || null, req.params.id]);
...
router.post("/study-plans/:id/courses", auth_1.verifyToken, (0, auth_1.requireRole)("admin"), async (req, res) => {
    try {
...
router.put("/study-plans/:id/courses/:courseId", auth_1.verifyToken, (0, auth_1.requireRole)("admin"), async (req, res) => {
    try {
        const { yearNo, semesterNo, isRequired, courseBucket } = req.body;
        await query("UPDATE study_plan_courses SET year_no = ?, semester_no = ?, is_required = ?, course_bucket = ? WHERE plan_id = ? AND course_id = ?", [
            yearNo,
            semesterNo,
...
router.delete("/study-plans/:id/courses/:courseId", auth_1.verifyToken, (0, auth_1.requireRole)("admin"), async (req, res) => {
    try {
        await query("DELETE FROM study_plan_courses WHERE plan_id = ? AND course_id = ?", [req.params.id, req.params.courseId]);
        return res.json({
            success: true,
...
router.post("/", auth_1.verifyToken, (0, auth_1.requireRole)("admin"), async (req, res) => {
    try {
        ...
        const result = await query("INSERT INTO courses (course_code, course_title, credits, description, department_id, program_id) VALUES (?, ?, ?, ?, ?, ?)", [
            courseCode,
            courseTitle,
            credits,
            description,
...
router.put("/:id", auth_1.verifyToken, (0, auth_1.requireRole)("admin"), async (req, res) => {
    try {
        const { courseCode, courseTitle, credits, description, departmentId, programId, } = req.body;
        await query("UPDATE courses SET course_code=?, course_title=?, credits=?, description=?, department_id=?, program_id=? WHERE course_id=?", [
            courseCode,
            courseTitle,
            credits,
            description,
