...
const ensureProfessorCanAccessSection = async (userId, sectionId) => {
...
const getSectionComponents = async (sectionId) => {
    const rows = await query(`SELECT component_id, section_id, component_name, weight, display_order
     FROM grade_components
     WHERE section_id = ?
     ORDER BY display_order, component_id`, [sectionId]);
    return rows.map((row) => ({
...
const validateComponents = (components) => {
    ...
    if (normalized.some((component) => !Number.isFinite(component.weight) || component.weight < 0)) {
        return "Each grade component must have a valid weight.";
    }
    const total = normalized.reduce((sum, component) => sum + component.weight, 0);
    if (total !== 100) {
        return "All component weights together must equal 100.";
    }
    return null;
};
const syncLegacyGradeControlFields = async (sectionId, userId, components) => {
...
const calculateAndUpdateGPA = async (studentId) => {
    const gradeRows = await query(`SELECT g.letter_grade, c.credits, cs.semester, cs.year
     FROM grades g
    ...
    for (const row of gradeRows) {
        ...
        if (!semesterMap[key])
            semesterMap[key] = { weighted: 0, credits: 0 };
...
const buildStudentGradeRecords = async (studentId) => {
    const grades = await query(`SELECT g.grade_id AS id, g.section_id, g.total_score, g.letter_grade,
     ...
     JOIN courses c ON cs.course_id = c.course_id
     WHERE g.student_id = ?
     ORDER BY cs.year DESC, cs.semester`, [studentId]);
    const componentScores = await query(`SELECT g.grade_id, gc.component_id, gc.component_name, gc.weight, gc.display_order, gcs.score
     FROM grades g
     JOIN grade_component_scores gcs ON g.grade_id = gcs.grade_id
     JOIN grade_components gc ON gcs.component_id = gc.component_id
     WHERE g.student_id = ?
     ORDER BY gc.display_order, gc.component_id`, [studentId]);
    const byGradeId = new Map();
    componentScores.forEach((row) => {
...
router.get("/my-gpa", auth_1.verifyToken, (0, auth_1.requireRole)("student"), async (req, res) => {
    try {
        const studentId = await getStudentId(req.user.id);
        if (!studentId)
            return res
...
router.get("/:studentId/gpa", auth_1.verifyToken, async (req, res) => {
    try {
...
router.get("/section/:sectionId/setup", auth_1.verifyToken, (0, auth_1.requireRole)("professor", "admin"), async (req, res) => {
    try {
       ...
       ORDER BY u.last_name, u.first_name`, [sectionId, sectionId]);
        const componentScores = await query(`SELECT g.student_id, gcs.component_id, gcs.score
       FROM grades g
       JOIN grade_component_scores gcs ON g.grade_id = gcs.grade_id
       WHERE g.section_id = ?`, [sectionId]);
        const scoreMap = new Map();
        componentScores.forEach((row) => {
            scoreMap.set(`${row.student_id}:${row.component_id}`, Number(row.score));
        });
...
router.put("/section/:sectionId/setup", auth_1.verifyToken, (0, auth_1.requireRole)("professor", "admin"), async (req, res) => {
    try {
        const { sectionId } = req.params;
        if (req.user.role === "professor") {
            const allowed = await ensureProfessorCanAccessSection(req.user.id, sectionId);
        ...
        if (existingGrades.length > 0) {
            await query(`DELETE gcs FROM grade_component_scores gcs
...
router.get("/", auth_1.verifyToken, (0, auth_1.requireRole)("admin", "professor"), async (req, res) => {
    ...
    `;
        ...
        const gradeIds = gradeRows.map((row) => Number(row.grade_id));
        let componentRows = [];
        if (gradeIds.length > 0) {
            componentRows = await query(`SELECT gcs.grade_id, gc.component_id, gc.component_name, gc.weight, gc.display_order, gcs.score
         FROM grade_component_scores gcs
         JOIN grade_components gc ON gcs.component_id = gc.component_id
         WHERE gcs.grade_id IN (${gradeIds.map(() => "?").join(",")})
         ORDER BY gc.display_order, gc.component_id`, gradeIds);
        }
...
router.post("/", auth_1.verifyToken, (0, auth_1.requireRole)("professor", "admin"), async (req, res) => {
    try {
        ...
        (Array.isArray(componentScores) ? componentScores : []).forEach((entry) => {
            ...
            if (componentId)
                scoreMap.set(componentId, score);
        });
       ...
       LIMIT 1`, [studentId, sectionId]);
        ...
        for (const component of components) {
            const score = scoreMap.get(component.component_id) ?? 0;
            await query(`INSERT INTO grade_component_scores (grade_id, component_id, score)
         VALUES (?, ?, ?)
         ON DUPLICATE KEY UPDATE score = VALUES(score)`, [gradeId, component.component_id, score]);
        }
