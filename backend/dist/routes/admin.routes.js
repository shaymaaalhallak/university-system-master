...
router.post("/grade-entry/:sectionId/mode", auth_1.verifyToken, (0, auth_1.requireRole)("admin"), async (req, res) => {
    try {
        ...
        await query("UPDATE grade_entry_control SET entry_mode = ? WHERE section_id = ?", [entry_mode, req.params.sectionId]);
...
router.post("/grade-entry/bulk", auth_1.verifyToken, (0, auth_1.requireRole)("admin"), async (req, res) => {
    try {
        ...
        if (action === "enable") {
            ...
            for (const sectionId of sectionIds) {
                await query(`INSERT INTO grade_entry_control (section_id, is_enabled, enabled_by, entry_mode, close_at)
           VALUES (?, 1, ?, ?, ?)
           ON DUPLICATE KEY UPDATE is_enabled = 1, enabled_by = VALUES(enabled_by), entry_mode = VALUES(entry_mode), close_at = VALUES(close_at)`, [sectionId, req.user.id, entry_mode || null, close_at || null]);
            }
        }
        else {
            for (const sectionId of sectionIds) {
                await query(`INSERT INTO grade_entry_control (section_id, is_enabled, enabled_by)
           VALUES (?, 0, ?)
           ON DUPLICATE KEY UPDATE is_enabled = 0, enabled_by = VALUES(enabled_by)`, [sectionId, req.user.id]);
            }
        }
