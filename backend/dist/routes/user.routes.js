...
ORDER BY cs.year DESC, cs.semester`, [professorId]),
            Promise.all([
                safeList(`SELECT a.assignment_id, a.title, a.description, a.due_date, a.created_at, a.max_score,
                    a.attachment_url,
                    c.course_code, c.course_title
             FROM assignments a
             ...
             JOIN courses c ON cs.course_id = c.course_id
             WHERE cs.professor_id = ?
             ORDER BY a.due_date DESC`, [professorId]),
                safeList(`SELECT e.exam_id, e.exam_date, e.exam_type, e.max_marks, e.section_id,
                    c.course_code, c.course_title
             FROM exams e
             JOIN course_sections cs ON e.section_id = cs.section_id
             JOIN courses c ON cs.course_id = c.course_id
             WHERE cs.professor_id = ?
             ORDER BY e.exam_date DESC`, [professorId]),
...
router.get("/professors/:id/performance-details", auth_1.verifyToken, (0, auth_1.requireRole)("admin"), async (req, res) => {
    try {
        ...
        const profRow = await safeList("SELECT professor_id FROM professors WHERE user_id = ?", [userId]);
        if (profRow.length === 0) {
        ...
        const sections = await safeList(`SELECT cs.section_id, cs.course_id, cs.semester, cs.year, c.course_code, c.course_title,
                ${sectionNameCol} AS section_name, ${roomCol} AS room, ${scheduleCol} AS schedule
         FROM course_sections cs
         JOIN courses c ON cs.course_id = c.course_id
         WHERE cs.professor_id = ?
         ORDER BY c.course_code, cs.semester`, [professorId]);
        console.log("[PERF] Sections for professor", professorId, ":", sections.length);
        const results = [];
        for (const section of sections) {
            const components = await safeList(`SELECT component_id, component_name, weight, display_order
           FROM grade_components
           WHERE section_id = ?
           ORDER BY display_order`, [section.section_id]);
            ...
            for (const comp of components) {
                const scoreRows = await safeList(`SELECT gcs.score
             ...
             WHERE gc.section_id = ? AND g.section_id = ? AND gc.component_id = ?`, [section.section_id, section.section_id, comp.component_id]);
