...
router.get("/my", auth_1.verifyToken, (0, auth_1.requireRole)("student"), async (req, res) => {
    try {
        ...
        const rows = await query(`SELECT ex.exemption_id, ex.exam_id, ex.reason, ex.status, ex.admin_note, ex.requested_at,
       ...
       WHERE ex.student_id = ?
       ORDER BY ex.requested_at DESC`, [studentId]);
...
router.post("/", auth_1.verifyToken, (0, auth_1.requireRole)("student"), async (req, res) => {
    try {
        ...
        const existing = await query("SELECT exemption_id FROM exam_exemptions WHERE student_id = ? AND exam_id = ?", [studentId, examId]);
        if (existing.length > 0) {
            return res.status(400).json({ success: false, message: "You already submitted an exemption request for this exam" });
        }
        const result = await query("INSERT INTO exam_exemptions (student_id, exam_id, reason) VALUES (?, ?, ?)", [studentId, examId, reason]);
...
router.put("/:id", auth_1.verifyToken, (0, auth_1.requireRole)("admin"), async (req, res) => {
    try {
        ...
        await query("UPDATE exam_exemptions SET status = ?, admin_note = ? WHERE exemption_id = ?", [status, adminNote || null, req.params.id]);
