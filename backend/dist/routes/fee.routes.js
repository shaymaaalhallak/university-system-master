...
router.get("/structure", auth_1.verifyToken, async (_req, res) => {
    return res.json({
        success: true,
        data: {
            tuition: {
...
router.get("/config", auth_1.verifyToken, (0, auth_1.requireRole)("admin"), async (_req, res) => {
    try {
...
router.post("/config", auth_1.verifyToken, (0, auth_1.requireRole)("admin"), async (req, res) => {
    try {
        ...
        await query(`INSERT INTO program_fee_settings (program_id, price_per_credit, effective_from, effective_to)
       VALUES (?, ?, ?, ?)
       ON DUPLICATE KEY UPDATE price_per_credit = VALUES(price_per_credit),
                               effective_from = VALUES(effective_from),
                               effective_to = VALUES(effective_to)`, [program_id, price_per_credit, effective_from || null, effective_to || null]);
...
router.delete("/config/:id", auth_1.verifyToken, (0, auth_1.requireRole)("admin"), async (req, res) => {
    try {
...
// ─── Invoices ──────────────────────────────────────────────────────────────
router.get("/invoices", auth_1.verifyToken, (0, auth_1.requireRole)("admin"), async (req, res) => {
    try {
...
router.get("/invoices/my", auth_1.verifyToken, (0, auth_1.requireRole)("student"), async (req, res) => {
    try {
        const studentId = await getStudentId(req.user.id);
        if (!studentId)
            return res.status(404).json({ success: false, message: "Student profile not found" });
        const rows = await query("SELECT * FROM student_invoices WHERE student_id = ? ORDER BY year DESC, semester", [studentId]);
        return res.json({ success: true, data: rows });
    }
    catch (error) {
        console.error(error);
...
router.post("/invoices/generate", auth_1.verifyToken, (0, auth_1.requireRole)("admin"), async (req, res) => {
    try {
        ...
        let students;
        if (studentId) {
            students = await query("SELECT s.student_id, s.program_id, s.department_id FROM students s WHERE s.student_id = ?", [studentId]);
        }
        else {
      ...
      `, [semester, year]);
        ...
        for (const student of students) {
            ...
            const existing = await query("SELECT id FROM student_invoices WHERE student_id = ? AND semester = ? AND year = ?", [student.student_id, semester, year]);
            ...
            if (student.program_id) {
                const feeConfig = await query("SELECT price_per_credit FROM program_fee_settings WHERE program_id = ? AND (effective_to IS NULL OR effective_to >= CURDATE()) ORDER BY effective_from DESC LIMIT 1", [student.program_id]);
      ...
      `, [student.student_id, semester, year]);
            ...
            const discountsResult = await query("SELECT COALESCE(SUM(value), 0) AS total_discount FROM student_discounts WHERE student_id = ? AND semester = ? AND year = ?", [student.student_id, semester, year]);
            const discountAmount = Number(discountsResult[0]?.total_discount || 0);
            // Get existing penalties for this student/semester
            const penaltiesResult = await query("SELECT COALESCE(SUM(amount), 0) AS total_penalty FROM fee_penalties WHERE student_id = ? AND semester = ? AND year = ?", [student.student_id, semester, year]);
            const penaltyAmount = Number(penaltiesResult[0]?.total_penalty || 0);
            const finalAmount = totalAmount - discountAmount + penaltyAmount;
            const result = await query(`INSERT INTO student_invoices (student_id, semester, year, total_credits, price_per_credit, total_amount, discount_amount, penalty_amount, final_amount, status)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')`, [student.student_id, semester, year, totalCredits, pricePerCredit, totalAmount, discountAmount, penaltyAmount, finalAmount]);
...
router.put("/invoices/:id", auth_1.verifyToken, (0, auth_1.requireRole)("admin"), async (req, res) => {
    try {
...
// ─── Payments ──────────────────────────────────────────────────────────────
router.get("/payments", auth_1.verifyToken, (0, auth_1.requireRole)("admin"), async (req, res) => {
    try {
...
router.get("/payments/my", auth_1.verifyToken, (0, auth_1.requireRole)("student"), async (req, res) => {
    try {
        const studentId = await getStudentId(req.user.id);
        if (!studentId)
            return res.status(404).json({ success: false, message: "Student profile not found" });
        const rows = await query("SELECT * FROM student_payments WHERE student_id = ? ORDER BY payment_date DESC", [studentId]);
        return res.json({ success: true, data: rows });
    }
    catch (error) {
        console.error(error);
...
router.post("/payments", auth_1.verifyToken, (0, auth_1.requireRole)("admin"), async (req, res) => {
    try {
        ...
        const result = await query("INSERT INTO student_payments (invoice_id, student_id, amount, payment_method, transaction_reference, status, admin_notes) VALUES (?, ?, ?, ?, ?, 'completed', ?)", [invoice_id, student_id, amount, payment_method || null, transaction_reference || null, admin_notes || null]);
        // Update invoice paid_amount and status
        await query(`UPDATE student_invoices si
       SET si.paid_amount = COALESCE((SELECT SUM(amount) FROM student_payments WHERE invoice_id = ? AND status = 'completed'), 0),
           si.status = CASE
             ...
             ELSE 'pending'
           END
       WHERE si.id = ?`, [invoice_id, invoice_id, invoice_id, invoice_id]);
...
router.put("/payments/:id/verify", auth_1.verifyToken, (0, auth_1.requireRole)("admin"), async (req, res) => {
    try {
       ...
       SET si.paid_amount = COALESCE((SELECT SUM(amount) FROM student_payments WHERE invoice_id = ? AND status = 'completed'), 0),
           si.status = CASE
             ...
             ELSE 'pending'
           END
       WHERE si.id = ?`, [p.invoice_id, p.invoice_id, p.invoice_id, p.invoice_id]);
...
// ─── Discounts ─────────────────────────────────────────────────────────────
router.get("/discounts", auth_1.verifyToken, (0, auth_1.requireRole)("admin"), async (req, res) => {
    try {
        const rows = await query(`
...
router.post("/discounts", auth_1.verifyToken, (0, auth_1.requireRole)("admin"), async (req, res) => {
    try {
        ...
        const result = await query("INSERT INTO student_discounts (student_id, type, value, reason, semester, year) VALUES (?, ?, ?, ?, ?, ?)", [student_id, type, value, reason || null, semester, year]);
...
router.delete("/discounts/:id", auth_1.verifyToken, (0, auth_1.requireRole)("admin"), async (req, res) => {
    try {
...
// ─── Penalties ─────────────────────────────────────────────────────────────
router.get("/penalties", auth_1.verifyToken, (0, auth_1.requireRole)("admin"), async (req, res) => {
    try {
        const rows = await query(`
...
router.post("/penalties", auth_1.verifyToken, (0, auth_1.requireRole)("admin"), async (req, res) => {
    try {
        ...
        const result = await query("INSERT INTO fee_penalties (student_id, amount, reason, semester, year) VALUES (?, ?, ?, ?, ?)", [student_id, amount, reason || null, semester, year]);
        return res.status(201).json({ success: true, data: { id: result.insertId } });
    }
    catch (error) {
        console.error(error);
...
router.delete("/penalties/:id", auth_1.verifyToken, (0, auth_1.requireRole)("admin"), async (req, res) => {
    try {
...
router.get("/dashboard", auth_1.verifyToken, (0, auth_1.requireRole)("student"), async (req, res) => {
    try {
        const studentId = await getStudentId(req.user.id);
        if (!studentId)
            return res.status(404).json({ success: false, message: "Student profile not found" });
        const student = await query(`SELECT s.*, pr.program_name FROM students s
       LEFT JOIN programs pr ON s.program_id = pr.program_id
    ...
    `, [studentId]);
        // Get invoices
        const invoices = await query("SELECT * FROM student_invoices WHERE student_id = ? ORDER BY year DESC, semester DESC", [studentId]);
        // Get payments
        const payments = await query("SELECT * FROM student_payments WHERE student_id = ? ORDER BY payment_date DESC", [studentId]);
        // Get discounts
        const discounts = await query("SELECT * FROM student_discounts WHERE student_id = ? ORDER BY created_at DESC", [studentId]);
        // Get penalties
        const penalties = await query("SELECT * FROM fee_penalties WHERE student_id = ? ORDER BY created_at DESC", [studentId]);
